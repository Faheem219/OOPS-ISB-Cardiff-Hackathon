...
def init_llm():
    ...
    guard = Guard.from_rail("Backend/rails/security_education_guardrails.rail")
...
def build_retrieval_chain():
    ...
    print("[build_retrieval_chain] Building RAG chain")
...
def process_prompt(db, user_id: str, prompt: str) -> str:
    ...
    print(f"[process_prompt] Guardrails pre-check: {guard_pre}")
    ...
    global conversation_retrieval_chain
    print(f"[process_prompt] user_id={user_id} prompt={prompt[:50]}")
    ...
    print("[process_prompt] User object found")
    history = user_obj.get("chat_history", [])
    print("[process_prompt] Chat history length:", len(history))
    ...
    print("[process_prompt] Context length:", len(ctx))

    full_prompt = f"{SYSTEM_PROMPT}\n{ctx}User: {prompt}"
    print("[process_prompt] Full prompt length:", len(full_prompt))

    # 4) call LLM
    result = conversation_retrieval_chain({"query": full_prompt})
    raw_answer = result.get("result", "")
    print("[process_prompt] Raw answer:", raw_answer[:20])

    # 5) guardrails
    try:
        parsed = guard.parse(raw_answer)
        print("[process_prompt] Guardrails parse result:", parsed)
        ...
        if parsed.validation_passed and parsed.validated_output:
            ...
            print("[process_prompt] Guardrails wrapping raw answer into JSON:", wrapper)
    ...
    print("[process_prompt] db update:", update_result.raw_result)
